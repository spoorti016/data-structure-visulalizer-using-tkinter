...
#config is used to change the property of the widget
root.config(bg="Black")
#StringVar() is used to edit the widget's text
...
def generate():
    ...
    data = []
    #when we are not interested in values returned by the function we use _ in place of variable name
    #i.e here u are nnot interested how many times the loop is run till now just that it should run specific times overall
...
def drawData(data, colorlist):
    #delete("all") function will clear the canvas
    canvas.delete("all")
    can_height = 380
    can_width = 550
    #width of single bar
    ...
    # enumerate -adds a counter to an iterable and returns it in the form of enumerating object
...
#Combobox has a numeric data type
algmenu = ttk.Combobox(
    Mainframe, textvariable=select_alg, values=["Bubble Sort","Quick Sort"])
...
# digits-If the control variable used to control the scale data is of string type, this option is used to
# specify the number of digits when the numeric scale is converted to a string.
...
# It provides a sliding bar through which we can select the values by sliding from left to right or
# top to bottom depending upon the orientation of our sliding bar.
